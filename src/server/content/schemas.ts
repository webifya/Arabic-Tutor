...
export const multipleChoicePrivateSchema=z.object({...base,prompt:text.max(1000),options:z.array(z.object({id:key,label:text.max(500)})).min(2).max(8),correctOptionId:key,explanation:text.max(2000).optional()}).superRefine((value,ctx)=>{if(!value.options.some(option=>option.id===value.correctOptionId))ctx.addIssue({code:"custom",message:"Correct option is not present"});if(new Set(value.options.map(option=>option.id)).size!==value.options.length)ctx.addIssue({code:"custom",message:"Option IDs must be unique"});});
export type MultipleChoicePrivate=z.infer<typeof multipleChoicePrivateSchema>;
...
const payloadSchemas={
  ...
  multiple_choice:multipleChoicePrivateSchema,continue:z.object({...base,label:text.max(200)}),
} as const;
...
export function toPublicMultipleChoice(payload:MultipleChoicePrivate){return{schemaVersion:payload.schemaVersion,prompt:payload.prompt,options:payload.options};}
export const answerSchema=z.object({lessonId:z.string().min(1).max(32),blockId:z.string().min(1).max(32),selectedOptionId:key});
